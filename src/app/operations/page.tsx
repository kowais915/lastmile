...
export default async function OperationsPage() {
  ...
  return <main className="min-h-screen bg-[#f7f6f2] text-[#18231e]">
    ...
    <div className="mx-auto max-w-6xl px-6 pb-14">
      ...
      <section className="mt-7 rounded-[1.75rem] border border-[#e5e2d9] bg-white p-6 shadow-sm"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold tracking-[0.16em] text-[#65806b]">SEVEN-DAY OPERATIONAL PULSE</p><h2 className="mt-2 text-2xl font-semibold">Delivery output and dispatch volume</h2><p className="mt-2 text-sm text-[#68776d]">Delivered portions and confirmed handoffs from this workspace&apos;s Neon records.</p></div><span className="rounded-full bg-[#edf4eb] px-3 py-1.5 text-xs font-bold text-[#287047]">Live from Neon</span></div><div className="mt-5"><OperationsTrendChart data={intelligence.deliveryTrend} /></div></section>

      <section className="mt-7 grid items-start gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,.7fr)]">
        <section className="rounded-[1.75rem] border border-[#e5e2d9] bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold tracking-[0.16em] text-[#65806b]">AUTOMATED SIGNAL QUEUE</p><h2 className="mt-2 text-2xl font-semibold">What needs a human next</h2><p className="mt-2 text-sm text-[#68776d]">Expiry windows, unsigned plans, pickup claims, and stalled handoffs.</p></div><span className="rounded-full bg-[#edf4eb] px-3 py-1.5 text-xs font-bold text-[#287047]">{signals.length} live</span></div>
          {signals.length ? <div className="mt-5 max-h-[470px] space-y-2 overflow-y-auto pr-1" aria-label="Operational signals">{signals.map((signal) => <article key={`${signal.title}-${signal.dueAt}`} className="grid gap-4 rounded-2xl border border-[#e5e2d9] bg-[#fffefd] p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${severityStyles[signal.severity]}`}>{signal.severity}</span><p className="font-semibold text-[#24342b]">{signal.title}</p></div><p className="mt-2 text-sm leading-5 text-[#68776d]">{signal.detail}</p><p className="mt-2 text-xs font-semibold text-[#65806b]">Deadline: {dateTime(signal.dueAt)}</p></div><Link href={signal.href} className="shrink-0 rounded-xl bg-[#183d2a] px-4 py-2.5 text-center text-sm font-bold text-white transition hover:bg-[#285d3c]">Take action</Link></article>)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-[#cfd8d0] bg-[#f8faf6] p-7 text-center"><p className="font-semibold">No time-sensitive signals right now.</p><p className="mt-2 text-sm text-[#68776d]">The rules will surface new risk as operations data changes.</p></div>}</section>

        <aside className="space-y-4 xl:sticky xl:top-5"><section className="rounded-[1.75rem] border border-[#e5e2d9] bg-white p-6 shadow-sm"><p className="text-xs font-bold tracking-[0.16em] text-[#65806b]">AUTOMATION BOUNDARIES</p><h2 className="mt-2 text-2xl font-semibold">Fast, not automatic.</h2><ol className="mt-5 space-y-4 text-sm"><li className="flex gap-3"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#183d2a] text-xs font-bold text-white">1</span><span><b>Detect</b><br /><span className="text-[#68776d]">The system monitors windows and handoff status.</span></span></li><li className="flex gap-3"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#183d2a] text-xs font-bold text-white">2</span><span><b>Surface</b><br /><span className="text-[#68776d]">Urgent work is routed to the correct operational screen.</span></span></li><li className="flex gap-3"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#183d2a] text-xs font-bold text-white">3</span><span><b>Confirm</b><br /><span className="text-[#68776d]">A coordinator approves allocation and dispatch every time.</span></span></li></ol></section><section className="rounded-[1.75rem] border border-[#e5e2d9] bg-white p-6 shadow-sm"><p className="text-xs font-bold tracking-[0.16em] text-[#65806b]">ON-TIME DELIVERY</p><p className="mt-2 text-3xl font-bold text-[#183d2a]">{metrics.onTimeDeliveryRate === null ? "—" : `${metrics.onTimeDeliveryRate}%`}</p><p className="mt-2 text-sm leading-6 text-[#68776d]">Of delivered handoffs, this share reached a partner before the recorded expiry deadline.</p></section></aside>
      </section>
